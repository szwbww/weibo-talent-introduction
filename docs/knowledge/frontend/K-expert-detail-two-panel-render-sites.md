...
revalidated_by: create-p:04-expert-list-ui-refinement
severity: P1
---
...
- 切换共用：`activateDetailSubTab(btn)` `app.js:7127-7150`，用
...
- panel DOM 各写各的（grep `detail-tab-panel` 回执）：
  - `showExpertDetail()` `:7195` → `:7230` / `:7233` / `:7305` / `:7308`，默认激活 `academic`
  - `loadContactDetail()` `:7617` → `:7727` / `:7730` / `:7871` / `:7874`，默认激活 `contact`
- 分流点：`handleContactAction` 的 `select-expert` 分支按 `expert?.contactId` 二选一（`app.js:9207-9219`）。
- 事件绑定共用：`#contactDetail` 上的 click 委托（`app.js:11790-11806`），
  `closest("[data-sub-tab]")` → `activateDetailSubTab`。新标签自动继承，无需新增绑定；
 但**只监听了 click**，需要 `change`（如面板内下拉）时要另加一个 change 委托
 —— 现已存在一个：`app.js:11807-11816`，只处理 `[data-role="mail-preview-template"]`。
...
（`showExpertDetail` 在 `:7208-7209` 把它清空并隐藏），该区域沿用全局 id 是安全的。
