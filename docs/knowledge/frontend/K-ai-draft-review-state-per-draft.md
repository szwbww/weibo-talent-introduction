...
正确做法：每个 draft entry 必须与 raw/rendered 一起保存自身的 `usedLlm/generationState` 采用边界；未经过 LLM 的 fallback 永远不可采用。当前“采用后直接人工发送”仍不把该状态传成历史发送审批；若未来重新启用逐项审核，再额外保存该草稿自己的 review snapshot，不能复用最后一次 response。
