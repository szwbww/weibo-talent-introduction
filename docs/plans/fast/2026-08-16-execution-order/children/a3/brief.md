# Fast-P Child Brief — a3

- Child: a3
- Plan: docs/plans/2026-08-16/a3-expert-list-rename-and-entry-move.md
- Plan identity: commit:65b8de831a5f0edeafeae5683a2f15b79f7000a3
- Depends on: a1,a2
- Base: bb07586b758357ad21794e17b7e99f200abeed5b  (a2 terminal Code head)
...
2. Cache key triad (M-2 / K-frontend-cache-key-triad): this child must set all three ?v= values in index.html to `20260817-v3-expert-list-entry-move` AND update the three literal string assertions in src/test/js/batchSendTaskConsoleVisualFix.test.js to the same value. Chain authority: a2 set them to 20260817-v2-batch-manual-log-entry; verify current values equal v2 BEFORE editing (if not, stop and report — someone skipped order).
3. No Flyway migration for this child (A family has none). Do not touch src/main/resources/db/migration/.
4. Next child b1 depends on this child's terminal code head and bumps cache keys to v4 (20260817-v4-task-records-paging).
5. Git: commit locally only, exactly one implementation commit with message `feat(fast-p): implement a3`. Never push, merge, rebase, amend, or rewrite history. Exclude fast-p report/log files (docs/plans/fast/) from the commit.

## Authorized files (from the plan 变更文件清单 — 4 files, modify nothing else)

1. src/main/resources/static/index.html   (nav text 专家联系->专家列表; delete line 591 button row; insert that button into mailbox panel title bar after the <h2>; three cache keys to v3)
2. src/main/resources/static/app.js   (viewMeta.contacts title text ONLY — exactly 1 line changed, S3-1; the four bulkOutreachBtn references stay untouched, M-4)
3. src/test/js/batchSendTaskConsoleVisualFix.test.js   (three cache key assertions)
4. src/test/js/batchEntryRelocation.test.js   (NEW)

NO styles.css changes. expertTagBatchFix.test.js NOT authorized (cut keeps its count-1 assertion green; must remain green though — run it).

## Required commands (run all; from plan 验证命令 + family main plan X-3)

- node --test src/test/js/batchEntryRelocation.test.js   (new file)
...
None marked (no ⏳ / ⚠️ 执行前须核实 entries). Plan line numbers (index.html:106/591/730, app.js:514) must be re-verified by grep before editing (K-plan-quantified-claims-need-grep-receipts).

## Downstream interfaces

- b1 (next child) reads this child's Code head as its Base and bumps cache keys to v4. B4 (child 7) will add a filter hint bar in the mailbox panel that must coexist with the relocated #bulkOutreachBtn — keep the button cut verbatim (M-4: exactly one occurrence; four app.js references unchanged) so B4's S2b-3 can locate it.
- Chain check: before editing, index.html cache values must equal v2 (20260817-v2-batch-manual-log-entry); after this child they must be v3.
