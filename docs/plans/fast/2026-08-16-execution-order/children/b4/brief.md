# Fast-P Child Brief — b4

- Child: b4
- Plan: docs/plans/2026-08-16/b4-task-drilldown-frontend.md
- Plan identity: commit:65b8de831a5f0edeafeae5683a2f15b79f7000a3
- Depends on: b3
- Base: eb27b8d84a4286ce3ef92ca40acf98d761168121  (b3 terminal Code head)
...
2. Cache key triad (M-7 / K-frontend-cache-key-triad): set all three ?v= values in index.html to `20260817-v6-task-drilldown` AND the three literal assertions in batchSendTaskConsoleVisualFix.test.js to the same value — in the SAME commit. Chain: b2 left v5 (20260817-v5-task-type-catalog); verify current values equal v5 BEFORE editing (if not, stop and report PLAN_CONFLICT).
3. No Flyway migration for this child (B4 does not touch schema). Do not touch src/main/resources/db/migration/.
4. Last child b5 depends on b1 (idx_te_started) and can be interleaved — it comes next regardless.
5. Git: commit locally only, exactly one implementation commit `feat(fast-p): implement b4`. Never push, merge, rebase, amend, rewrite. Exclude docs/plans/fast/ from the commit.

## Authorized files (from the plan 变更文件清单 — 10 files; file 5's exact name must be CONFIRMED first)

1. src/main/kotlin/com/weibo/talentintroduction/task/domain/TaskTypeCatalog.kt   (4 drilldown declarations)
2. src/main/kotlin/com/weibo/talentintroduction/task/controller/TaskExecutionController.kt   (/{id}/detail + 4 fields; reuse PollDetailRaw/PollDetailAccountRaw/PollDetailExpertRaw)
3. src/main/kotlin/com/weibo/talentintroduction/mail/repository/MailRecordRepository.kt   (1 derived query)
4. src/main/kotlin/com/weibo/talentintroduction/mail/service/MailboxService.kt   (listByTaskExecution, reuse assembly)
5. src/main/kotlin/com/weibo/talentintroduction/mail/controller/MailboxController.kt   (ACTUAL FILE NAME UNVERIFIED — grep first; list endpoint + optional param)
6. src/main/resources/static/app.js   (entry rendering + 2 actions + loadMailbox filter + hint bar)
7. src/main/resources/static/index.html   (S2b-3 hint bar)
8. src/test/kotlin/com/weibo/talentintroduction/mail/service/MailboxTaskExecutionFilterTest.kt   (NEW)
9. src/test/js/taskDrilldown.test.js   (NEW)
...
1. MailboxService research gap (plan line ~172): read the FULL list-query method in MailboxService BEFORE editing — confirm ① list vs detail queries share assembly; ② where tags (MAILBOX_TAG_BADGE_CLASS) are computed. Fill the plan's audit section into the execution report as the verbatim baseline.
2. File 5 controller name/endpoint UNVERIFIED: grep to locate the mailbox list endpoint's controller and method; if fixing it requires changes to 2+ backend files beyond the list, STOP and report PLAN_CONFLICT (plan: 停止并回报).
3. T2b-4 QUEUE marker: confirm the EXACT JSON field name in request_payload written by MailAutomationScheduler.dispatchMode() (TaskDispatchRequest.dispatchMode == "QUEUE") — do not guess (plan: 不得凭印象).
4. A3 region overlap: inspect the ACTUAL mailbox panel DOM (a3 moved #bulkOutreachBtn into the panel title bar); the S2b-3 hint bar goes BELOW the title bar, AFTER the existing .toolbar — must not share the button's row.
5. `.link-btn` margin-left:auto side effect: entry container must be block-level non-flex; do NOT modify the .link-btn rule block (grep its usage sites first if visual position is off).
6. N2b-2: other entries into the mailbox view (goto-manual-queue / open-pending) must clear the taskExecutionId filter state; the new filter state must be cleared on those paths (K-cross-view-drilldown-pattern).

## Downstream interfaces

- b5 (next child) is pure backend (task) and does not touch frontend files or cache keys; it needs idx_te_started (already created by b1). No conflict with this child's files.
- N-1..N-7 binding (recent-polls endpoints unchanged; /{id}/detail stays 200 for any taskType; N2b-5 view registration untouched — no new views).
- Chain check: index.html cache values must equal v5 before editing, v6 after.
