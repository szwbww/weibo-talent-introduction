...
- Resume from: c4, epoch 1, code head `e77cb065ba6261317adc7060b2a7729052086407`. Next action: supply the production authorization plus the maintenance window, then perform the stop-write re-collection, verifiable backup, manual equivalent V134 DDL, three-stage per-ID classification (internal probes first), in-transaction repair with the single `LuKai_QF.inbound_mailbox_code='LuKai'` update, and the controlled resume, recording everything in `docs/runbooks/repair-lukai-shared-inbox.md`. See `children/c4/brief.md` for the full gate analysis and `children/c4/execution.md` for the placeholder that a resumed executor must fill.
