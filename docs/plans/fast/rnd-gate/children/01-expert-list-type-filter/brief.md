# Fast-P Child Brief — 01-expert-list-type-filter

- Master: docs/plans/2026-08-25/00-rnd-gate-master.md (commit 2b80a92)
- Approved plan (full contract, verbatim below): docs/plans/2026-08-25/01-expert-list-type-filter.md (commit 2b80a92)
- Child product base: f2935072c819a9167e75220a6a959b0769462fde
...
- M-2: classification semantics single source — new code only CONSUMES ExpertClassificationService/ExpertClassification results; no copied keywords/thresholds/type inference anywhere.
- M-3: empty collection = no filter; `expertTypesFilter(emptyList())` MUST return null; never emit `should: [] + minimum_should_match: 1`.
- Master must-keep: existing `discipline` filter behavior byte-identical; this child is read-only (no write paths).
- Shared files with later children (sequential only, never parallel): `ExpertSearchService.kt`, `app.js`, `index.html` — 02 depends on 01's exact API.

## Downstream interfaces (02-batch-send-type-filter consumes these)
- `ExpertSearchService.ALLOWED_EXPERT_TYPES: Set<String>` in companion object = `ExpertType.values().map { it.name }.toSet() + "UNCLASSIFIED"`.
- `ExpertSearchService.expertTypesFilter(types: List<String>): Map<String, Any>?` (null on empty; single bool.should + minimum_should_match:1 otherwise).
- `buildExpertFilters(...)` and `searchExperts(...)` gain trailing `expertTypes: List<String> = emptyList()` — the three aggregation callsites (:949/:994/:1101) MUST remain byte-identical via defaults.
...
- Targeted: `JAVA_HOME=... mvn test -Dtest=ExpertSearchServiceTest`; `node --test src/test/js/expertTypeFilter.test.js`; `node --test src/test/js/*.test.js`; `node --check src/main/resources/static/app.js`; `git diff --check`.
