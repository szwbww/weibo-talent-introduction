# Fast-P Child Brief — trust-reply-configurable-workbench-03
...
- Child base SHA: c99c3aa (child 02 implementation, LIGHT_PASS)
...
- Master must-NOT-change list applies verbatim (request extraction/requestKey/intent/grounding; QA answerBody sole fact body; active vs resolved separation; locked answers canonical-order verbatim once; SIMULATION/LIVE fixed entries; ADJUST_ITEM-only autofill; server-side final reassembly; reply snippet/autoreply/matched/FREE_FORM/template default frame behavior; permissions, optimistic concurrency, expiry, payload limit, audit, history unchanged).
- Global invariants in force for this child: G-1 (canonical config = requestFactSelections + frameSnapshot end-to-end), G-2 (server-side uniqueness, 422 handled with canonical refresh), G-3 (client submits IDs only, shows server-resolved text), G-4 (fact change invalidates evidence/versions; frame change only assembly), G-5 (final reassembly carries same matrix/frame/locks), G-6 (client sends v3 schema literal only; v1/v2 readable), G-7 (two pages share one state machine; no per-keystroke DOM rebuild; full ARIA/keyboard).
- Style contract S-1..S-6 in the plan is THE authority for all new CSS: verbatim selectors/properties; global .tabs/.tab, .mailbox-segmented-control, .button, .compose-panel must have ZERO diff; new rules all scoped to .trust-reply-workbench; retire .trust-reply-layout and toolbar global fact rules per S-5.
- No DB changes; child 01 (requestFactSelections) and child 02 (frameSnapshot, frameOptions) backend contracts must be used as-is, never regressed.
- Required commands (run freshly, record exit codes in execution report):
  ...
  - git diff --check
- Baseline at child base c99c3aa: mvn test exit 0 (2118 run, 4 pre-existing skipped), node --test src/test/js/*.test.js exit 0 (413 pass), git diff --check clean.
- Commit the implementation locally as: feat(fast-p): implement trust-reply-configurable-workbench-03
- Exclude docs/plans/fast/** from the implementation commit (controller commits evidence separately).
- Write the full execution result to docs/plans/fast/trust-reply-configurable-workbench/children/03/execution.md (this file is OUTSIDE the implementation commit too).
- Use skill execute-p against the plan below. Do not review later children (none), repair unrelated behavior, push, merge, or rewrite history.

---

# 可信回复工作台 03：双页切换前端与全链路透传开发计划

> 使用 `create-p` 编写。总纲：`trust-reply-configurable-workbench-00-master.md`。前置：计划 01 的摘要—事实矩阵与计划 02 的可选 frame 后端契约已执行并通过独立验证。本计划落实 2026-08-05 已确认的“双页切换”预览，不使用左右分栏。
