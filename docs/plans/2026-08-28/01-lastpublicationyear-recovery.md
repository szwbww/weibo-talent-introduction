...
- 预期结果: `lastPublicationYearPending` 初值 **约 7.5 万**（2026-08-28 实测 **74,697**）；
  任务启动后 `totalCount` 等于该值。若初值 < 30,000 或 > 90,000，说明过滤器口径错误，停止并回报。

  > **2026-08-28 修正**：本条原写「35,000~45,000，>60,000 即停止」，是**计划缺陷**。
  > 41,409 那个数字是「**UNKNOWN 之中**缺年份的条数」，而补采口径覆盖的是**全部**
  > 有 `enrichedAt` 的文档 —— ACADEMIC_RND / OUT_OF_SCOPE 同样可能缺年份。
  > 交叉验算：`enrichedLast30d` 85,146 − 74,697 ≈ 10,449 有年份，
  > 即已 enrich 文档里 **87.7%** 缺年份，与此前在 UNKNOWN 子集上测到的 86.6% 一致，
  > 口径正确。按原阈值执行会**误判为口径错误并停止**。
- 覆盖: I1-5、需求描述第 2 条
