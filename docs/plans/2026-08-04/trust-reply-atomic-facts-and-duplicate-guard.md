...
可信回复面对同时询问“IP/作者署名/研究保密、正式合同、参与费用”的邮件时，每项只使用与该问题语义对应的原子事实：IP 项不夹带合同全文或费用，合同项不重复 IP 回答，费用项只回答费用政策。若两个已锁定项仍引用同一 intent + 同一事实，服务端整合拒绝并返回稳定错误码，不静默删改用户已锁定正文。
...
- Rule：一个新启用 QA 规则的 `answer_body/reply_body` 只能覆盖其声明的一个语义族。材料保密、费用政策、合同安排、签约前 IP 边界必须是四个独立规则；不得再次把“不收费”放入保密规则，或把完整合同条款放入 IP 规则。
- Applies to：V82 迁移新增规则、运行时 QA 管理 create/update。
...
- Rule：create command 的 `coverageKeys=null` 写空，非空列表经 `QaCoverageKeyCatalog.normalizeAndValidate` 后写入；update command 的 null 保留旧值，空列表显式清空，非空列表校验后替换。`answerBody` 与 `replyBody` 继续写同一规范正文。
- Applies to：`QaRuleManagementService.createRule/updateRule`。
...
- Write paths：
  ...
  2. `QaRuleManagementService.createRule` — repository save，当前强制 `coverageKeys=""`。
  3. `QaRuleManagementService.updateRule` — repository save，当前保留旧 coverage，忽略 command。
  4. `deleteRule/setRuleEnabled` — 删除或切换 enabled。
- Read paths：
...
### Task 3：补齐运行时 coverage_keys 写路径（I-1、I-2、I-5）
...
3. 保留 answerBody/replyBody 同源、reply policy、variants 禁止及 category 校验。
...
3. create/update/null/empty/invalid coverage 的持久化语义。
...
| 4 | `src/main/kotlin/com/weibo/talentintroduction/qa/service/QaRuleManagementService.kt` | 修改 | create/update 真正维护 coverage_keys |
| 5 | `src/main/kotlin/com/weibo/talentintroduction/llm/service/TrustReplyWorkbenchService.kt` | 修改 | assemble 前重复 claim/完全重复答案校验 |
| 6 | `src/test/kotlin/com/weibo/talentintroduction/llm/service/QaFactSelectionServiceTest.kt` | 修改 | 原子规则与 coverage 选择测试 |
| 7 | `src/test/kotlin/com/weibo/talentintroduction/qa/service/QaRuleManagementServiceTest.kt` | 修改 | coverage 写路径测试 |
| 8 | `src/test/kotlin/com/weibo/talentintroduction/llm/service/TrustReplyWorkbenchItemFlowTest.kt` | 修改 | 重复防线与 composer 保真回归 |

范围：8 文件；QA 事实选择 + 可信回复整合两个子系统。无前端/CSS/新表。

## 验收标准

- I-1：V82 中四条启用规则的正文与 coverage 一一对应；保密正文不含 fee，费用正文不含 confidentiality，合同正文不含 IP boundary，IP 正文不复制合同段。
...
- I-5：管理 service 对 create null/non-empty、update null/empty/non-empty/invalid 全覆盖。
...
### A-4：后台创建/编辑 coverage
- 前置条件：QA 管理页新建测试规则权限。
- 操作步骤：创建带 `fees.policy` 的规则；编辑正文但不提交 coverage；再显式清空 coverage。
- 预期结果：首次保存 coverage 为 `fees.policy`；第二次仍保留；显式清空后变空且不再支撑费用 intent。
- 覆盖：I-5、interaction qa_rule write→selection read。
