...
建立一个identityVerification对象及共享纯函数策略；三层显式mapping与profile读取一致，按邮箱/姓名绑定证据。来源识别以对象、emailSource、discovered标签共同兜底；缺少证明的深度发现历史记录不放行，其他来源不改变。新资源精确导出最终删除1923邮箱SHA256，不含排除专家。
...
三层ES dynamic:false，externalIds enabled:false；ExpertIndexService从mapping读取全部属性并更新；ExpertSearchService.sourceFields/toExpertProfile读取。晋升整份source复制。OperatorStatusWriteSeamGuardTest以行号匹配需机械同步。来源：K-expert-classification-one-object-three-layers。
...
| 文件 | 动作 |
|---|---|
| src/main/kotlin/com/weibo/talentintroduction/expert/domain/DiscoveryIdentity.kt | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/main/kotlin/com/weibo/talentintroduction/expert/domain/ExpertProfile.kt | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/main/kotlin/com/weibo/talentintroduction/expert/service/ExpertSearchService.kt | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/main/resources/es/orcid_info_raw.json | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/main/resources/es/orcid_info_candidate.json | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/main/resources/es/orcid_info_application.json | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/main/resources/discovery/identity-deletion-blocklist.sha256 | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
| src/test/kotlin/com/weibo/talentintroduction/expert/domain/DiscoveryIdentityTest.kt | 实现本阶段契约或对应回归；守卫测试仅机械行号修正 |
...
- 命令：`JAVA_HOME=/Library/Java/JavaVirtualMachines/zulu-11.jdk/Contents/Home mvn test -Dtest=DiscoveryIdentityTest,ExpertSearchServiceTest,OperatorStatusWriteSeamGuardTest -Dexec.skip=true`。
