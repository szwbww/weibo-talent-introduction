...
> create-p 子计划；受 [MAIN 总计划](00-batch-sender-filter-main.md) 的顺序、接口与发布门槛约束。只创建计划，不授权执行。依赖 [后端计划](01-batch-sender-filter-backend.md) 的 `senderAccountCodes` API。共享收件箱计划 01 也修改 `index.html`/`app.js`，实施前以合并后的当前文件为基线重查 DOM、资源键和测试，不覆盖其“收件箱归属”字段。
...
- 必须保持：当前模板、收件筛选、定时参数和手动差异判定；UI 现有颜色/布局/响应式样式；账号页的共享收件箱配置；停用账号不会被误表示为可发送。
...
### A-3：共享收件箱与停用状态
- 前置条件：A/B 共享一个 IMAP 收件箱，但 senderEmail 不同；B 已停用且原配置选了 B。
