...
### M-5：共享收件箱计划的文件和迁移顺序不得冲突
- 规则：共享收件箱 [01 配置计划](01-shared-inbox-configuration.md) 预留 V134，并修改 `index.html`、`app.js`、`FlywayMigrationIntegrationTest.kt`。本组 01 的 V135 必须在代码仓库及测试环境的 Flyway 序列中排在 V134 后；本组 02 必须在共享收件箱 01 的前端最终基线上修改。后计划只增自己的字段/控件，不覆盖对方的 owner 配置、缓存键或测试断言。生产 Flyway 当前关闭，生产 DDL 按各自发布审批和现场核验处理，本 MAIN 不授权执行。若实际版本或文件基线变了，先改计划，不自行猜迁移号或机械套旧行号。
- 适用：实施前检查、合并、迁移、静态资源构建。
- 违反后果：迁移乱序、共用 UI 文件覆盖、线上旧 JS 缓存。
- 来源：`01-shared-inbox-configuration.md:实现方案/变更文件清单`；当前本组子计划的 V135 与静态资源审计。
...
| 文件 | 既有共享收件箱计划 01 | 本组子计划 | 串行规则 |
|---|---|---|---|
| `src/test/kotlin/com/weibo/talentintroduction/campaign/repository/FlywayMigrationIntegrationTest.kt` | V134 断言 | 后端 01 的 V135/旧行断言 | 先合入 V134，再以当前版本改最新版本断言；定点 target 断言不动 |
| `src/main/resources/static/index.html` | owner 选择器、启用文案、资源键 | 前端 02 的批量任务 picker、资源键 | 先合入 owner UI，再按当时实际键统一 bump |
| `src/main/resources/static/app.js` | owner 选择/保存/回显 | 前端 02 的账号多选/快照 | 方法区分开改；后者不能用旧整文件覆盖前者 |

## 实现方案

### G-0：外部基线门槛（M-5）
- 先确认共享收件箱计划 01 已实施并验证，V134 已在代码仓库与测试环境的 Flyway 序列，`index.html`/`app.js` owner 配置仍在。生产是否已手工执行 V134 的等价 DDL，须到发布前现场核验，不能由本门槛推定。若 01 尚未实施，本组维持计划状态；要改执行顺序，先重写本组两个子计划的迁移与共享文件约束。

### G-1：后端 01（M-1～M-5）
- 仅按 [后端子计划](01-batch-sender-filter-backend.md) 的 10 文件清单实施；先跑其定向/迁移验证，再做 API 黑盒 A-1～A-6。A-5 使用已完成共享收件箱 01 的测试配置构造同一 IMAP owner 的 A/B 逻辑账号，不要求生产 alias 切换。记录实际字段 JSON、启动快照、两类型发信 code、绑定跳过结果与失败项。通过后状态 `IMPLEMENTED_NOT_RELEASABLE`，以此结果作为前端唯一接口基线。
...
### G-3：联合验证与发布（M-1～M-5）
- 在同一测试版本验证：定时配置保存→启动快照→INTRODUCTION 实际发件；手动从配置带入并修改→预估/启动快照→实际发件；MATERIAL_REMINDER 已绑定全跳过；共享 IMAP owner/alias 分开选择；旧配置 `[]` 保持全池；账号失效时不跨白名单兜底。核对真实 `index.html` DOM、JS payload、API 返回、数据库列和 `mail_record.sender_account_code`，不能只看单元测试。
...
- M-5：仓库和测试环境 V134 早于 V135；生产 DDL 有独立现场核验与审批；共享 owner UI/API 未丢；11 个资源键统一；三个共享文件均从前一阶段产物串行修改。各子计划文件数仍 ≤10。
...
### A-3：共享收件箱和失效账号
- 前置条件：A/B 共享一个 IMAP inbox，分别有不同 SMTP 发件地址；B 已停用且一份历史配置选了 B。
- 操作步骤：打开该配置与账号管理页；执行仅选 B 的任务，再将任务改为仅选 A 执行。
- 预期结果：账号管理页的共享 owner 配置仍在；批量选择器把 A/B 分成两项，B 原选值可见并标停用；仅选 B 时外发 0，仅选 A 时只 A 外发。
- 覆盖：M-2、M-4、M-5。
