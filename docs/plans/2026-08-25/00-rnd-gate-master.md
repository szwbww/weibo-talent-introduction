...
## 已识别但本轮不做：SBIR 接入（2026-08-25 决定挂起）

> **状态：挂起（PARKED）。01-04 四份子计划不依赖它，可独立开工与发布。**
>
> 挂起原因：三轮实测均未触达 SBIR 服务端。最后一轮定位到本机代理的 fake-ip
> （DNS 解析到 `198.18.23.131`，RFC 2544 保留段）+ 该域名 TLS 握手被重置，
> 而同一路径下 `api.openalex.org` / `api.crossref.org` 均返回 200。
> **失败原因在网络侧，不是 SBIR 服务端**，因此**不得**据此选择路径 C（"本轮不接"）——
> 那是一个业务决策，需要真实数据支撑，不能由网络故障代替。
>
> ## 2026-08-25 定性完成：地域/IP 封禁（生产服务器实测）
>
> 五轮排查后定性。**不是维护、不是缺 API key、不是路径错误、不是 UA 拦截。**
>
> | 观察 | 排除了什么 |
> |---|---|
> | 故意写错的路径也返回 403 | 路径问题 |
> | `x-amzn-ErrorType: ForbiddenException` + `x-amz-apigw-id` | 应用层返回（是 API Gateway 在拒） |
> | 主站 `www.sbir.gov` 同样 403，后端却是 `Server: awselb/2.0` | **两套不同基础设施同样拒绝 → 共同的上游访问策略** |
> | `X-Amz-Cf-Pop: HKG61-P1` | 请求经 CloudFront 香港边缘出去 |
>
> 症状随出口而变：本地 Mac `000`（连接未建立），生产服务器 `403`（服务端已响应并拒绝）。
> 详见 [[K-sbir-gov-geo-blocked]]。
>
> **待验证（成本极低）**：把生产服务器的 mihomo 切到美国节点重跑 `scripts/cp1-forbidden.sh`。
> 变 200 则数据可获取；仍 403 则封禁更严格，可正式收口。
>
> **架构结论（无论上一条测出什么都成立）**：SBIR 不做实时 API 源。
> 生产定时任务依赖「代理节点在线且落在允许地区」过于脆弱，且会静默失败。
> 改走**离线数据摄入**：在允许的地区下载一次 → 本地索引 → 运行时零网络依赖。
> 这与 PatentsView 的方案一致，两者性质相同（存量档案，非持续更新流）。
>
> ---
>
> **2026-08-25 早前记录**：`https://www.sbir.gov/data-resources` 在**浏览器中同样无法访问**。
> 这排除了 UA / TLS 指纹拦截（浏览器与 curl 的 TLS 栈不同，若是指纹问题浏览器应能打开），
> 判定为**整个 sbir.gov 域名在当前网络出口下不可达**（IP / 网络层）。
> 离线下载路径同样受阻；`scripts/cp1-why.sh` 的 B 组（浏览器请求头）已无需再跑。
>
> 解除挂起需要满足**任一**条件：
> 1. **换一个代理出口节点**后重试 —— 当前节点 IP 疑似被 sbir.gov 拒绝，这是成本最低的一次尝试；
> 2. 在境外网络环境下跑 `scripts/cp1-sbir.sh`，或手动下载
>    `https://www.sbir.gov/data-resources` 的 award 批量数据带回本地；
> 3. 由该数据回答 CP-1 的全部问题：字段全集、`pi_email` 填充率，
>    以及**决定路径 B 是否必要的那个关键比例**（`firm` 含研发岗位词
>    且 `research_area_keywords` 含生产主题词的占比）。
>
> **在拿到真实数据之前，不得选择路径 C（"本轮不接 SBIR"）** —— 那是业务决策，
> 不能由网络故障代替。当前状态是「未知」，不是「不做」。
>
> 下方分析全部基于官方字段文档，结论在数据到手前保持有效。
