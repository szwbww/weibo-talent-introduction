...
- A2b（同日第二次发布阻断修复）：改号后的初版 V129 仍以 `DROP CHECK chk_expert_material_code` 直删，生产 MySQL 为 **5.7.41**，`ALTER TABLE ... DROP CHECK` 是 8.0.16+ 语法，首次发布在该语句上报 `ERROR 1064 ... near 'CHECK chk_expert_material_code'` 并中止（该轮只写入 kit 台账 `multi_ai_kit_schema_history` 的 `applying` 行，DDL 一行未执行：生产全库 `information_schema.TABLE_CONSTRAINTS` 零 CHECK 行，`expert_material_status` 零数据行，台账行已按 kit 恢复流程删除）。修正内容：V129 改为「先经 `information_schema.TABLE_CONSTRAINTS` 探测约束是否存在，仅在存在时用动态 SQL 执行 DROP；`ADD CONSTRAINT ... CHECK` 沿用 V36 写法（5.7 解析并忽略，8.0.16+ 真实生效）」。因此本计划 I-1/I-2 的「CHECK 扩域」在生产 5.7 上不落地（与 V36/V112/V115/V119 既有 CHECK 迁移同命运），DB 层不再约束 `material_code`，该域在生产仅由应用层枚举保证；CHECK 语义仍由 `FlywayMigrationIntegrationTest`（mysql:8.0.36）验证。实测：本地 MySQL 5.7.44 与该文件 → 退出 0、12 码可写、未知码也可写（无 CHECK 可执行）；mysql:8.0.36 → DROP+ADD 生效、12 码可写、`REQ_UNKNOWN` 报 3819 拒绝。
