...
p=pathlib.Path('/tmp/expert-identity-restore-193');rows=json.load(open(p/'verified-identities.json'));current={r['id']:r['source'] for r in map(json.loads,open(p/'preflight.jsonl')) if r['kind']=='expert'}
...
aliases={name.lower():name for name in countries.values()};aliases['bosnia and herzegovina']='Bosnia and Herzegovina';aliases['perú']='Peru'
...
for r in rows:
 ...
 if not r['id'].startswith('EMAIL-'):
  ...
  matches=[a for a in w['authorships'] if norm(a['author']['display_name'])==norm(r['expectedName']) or norm(a.get('raw_author_name'))==norm(r['expectedName'])];assert len({a['author']['id'] for a in matches})==1
  matches=list({a['author']['id']:a for a in matches}.values());assert len(matches)==1
