...
backupBytes=(root/'before.json').read_bytes();assert hashlib.sha256(backupBytes).hexdigest()=='c6801eb31ab30983c20934b1d099d29d89cf2c762d1aabad605a5c4927b17946'
b=json.loads(backupBytes);byid={r['id']:r for r in plan};assert len(byid)==193
...
assert proc.returncode==0,proc.stderr.decode();classes=[json.loads(l) for l in proc.stdout.splitlines()];assert len(classes)==386
manifest=[]
