...
result={'verifiedAt':datetime.now().isoformat(),'status':'PASS' if not errors and mailSame and jobSame and sum(counts.values())==386 else 'FAIL','experts':193,'esDocs':len(readback),'layers':counts,'contacts':len(rows),'comparedPatchedFields':fields,'allOtherEsFieldsPreserved':not errors,'stableBusinessKeysPreserved':True,'mailUnchanged':mailSame,'academicJobRowsUnchanged':jobSame,'legacyOrcidReviewed':len(legacy),'wrongAcademicMetricsCleared':len(wrongLegacy),'validLegacyMetricsPreserved':len(legacy)-len(wrongLegacy),'correctOpenAlexIdentities':len(legacy),'countryUnknown':sum(r['patch']['country'] is None for r in m['plan']),'errors':errors,'backupPath':str(root/'before.json'),'backupSha256':hashlib.sha256((root/'before.json').read_bytes()).hexdigest(),'manifestSha256':hashlib.sha256((root/'manifest.json').read_bytes()).hexdigest(),'remainingLimitations':['1924 unresolved audit records unchanged; no deletion or quarantine applied','39 wrong ORCID-shaped legacy business keys preserved; UI must stop treating these keys as actual ORCID','1 legacy case has multiple OpenAlex observed ORCIDs; real ORCID left unset, verified OpenAlex author identity retained','3 affiliation countries lack explicit evidence and are null; source institutions restored']}
