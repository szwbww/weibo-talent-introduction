...
live=all_docs();assert len(live)==4204
...
try:
 ...
 save('apply-complete.json',{'time':datetime.now().isoformat(),'experts':2102,'esDocs':4204,'contacts':14,'mailUnchanged':True});print(json.dumps({'stage':'APPLIED','experts':2102,'esDocs':4204,'contacts':14,'mailUnchanged':True}),flush=True)
 ...
 current=all_docs();same=sum(current[k]['_source']==before[k]['_source'] for k in before);save('rollback-result.json',{'restoredOrUnchanged':same,'total':4204,'conflicts':conflicts});print(json.dumps({'stage':'ROLLED_BACK','restoredOrUnchanged':same,'conflicts':conflicts}),flush=True);raise
