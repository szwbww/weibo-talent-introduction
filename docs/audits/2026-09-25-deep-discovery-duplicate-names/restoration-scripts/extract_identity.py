...
sys.path.insert(0,'/tmp/deep-discovery-name-audit');from extract_strict import extract,norm,text
p=pathlib.Path('/tmp/expert-identity-restore-2102');cache=pathlib.Path('/tmp/deep-discovery-name-audit/public_xml')
rows=json.load(open('docs/audits/2026-09-25-deep-discovery-duplicate-names/confirmed-wrong.json'));out=[];parsed={}
def afftext(n):
 v=n.text or ''
 for c in n:
...
for r in rows:
 ...
 if pmc not in parsed:
  doc=ET.parse(cache/(pmc+'.xml')).getroot();meta=doc.find('./front/article-meta');parsed[pmc]=(meta,extract(cache/(pmc+'.xml')))
 meta,(mappings,doi,authors)=parsed[pmc];mapping=mappings[r['email']];assert len(mapping['names'])==1 and norm(mapping['names'][0])==norm(r['expectedName'])
 ...
 out.append({'id':r['id'],'email':r['email'],'oldName':r['name'],'expectedName':r['expectedName'],'givenNames':text(c.find('name/given-names')).strip(),'familyNames':text(c.find('name/surname')).strip(),'affiliations':affs,'verifiedOrcids':oids,'doi':doi,'paperYear':min(years) if years else None,'pmcId':pmc,'evidenceUrl':r['evidenceUrl']})
