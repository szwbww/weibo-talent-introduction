...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-contactout-review-badge] img').count(), 0);
    ...
    assert.equal(await page.locator('#junior').evaluate(c => getComputedStyle(c).backgroundColor), 'rgb(255, 241, 242)');
    assert.equal(await page.locator('#ordinary').evaluate(c => getComputedStyle(c).backgroundColor), 'rgb(255, 251, 235)');
    ...
    await page.waitForFunction(() => !document.querySelector('#collapsed [data-contactout-review-badge]') && !document.querySelector('#ordinary [data-contactout-review-badge]') && document.querySelector('#junior[data-contactout-review="review"]'));
    assert.equal(await page.locator('#junior[data-contactout-ignore]').count(), 0);
    await page.evaluate(() => markContactOutIgnored());
    assert.equal(await page.locator('#junior [data-contactout-review-badge]').count(), 1);
    assert.equal(await page.evaluate(() => reviewClicks), 0);
    await page.evaluate(() => clearContactOutIgnoreMarks());
    ...
    assert.match(await page.locator('#score70-low [data-contactout-review-badge]').textContent(), /Score 70 Corp 70分.*L2.*L3/);
    assert.equal(await page.locator('#score100-low').evaluate(c => getComputedStyle(c).backgroundColor), 'rgb(255, 241, 242)');
    assert.equal(await page.locator('#unlisted').evaluate(c => getComputedStyle(c).backgroundColor), 'rgb(255, 251, 235)');
    ...
    assert.match(await page.locator('#auto-one [data-contactout-review-badge]').textContent(), /Henkel 30分.*L1.*L4/);
    assert.deepEqual(await page.evaluate(() => autoClicks), ['auto-one:more']);
    await page.evaluate(() => { globalThis.firstAutoBadge = document.querySelector('#auto-one [data-contactout-review-badge]'); });
    await page.evaluate(() => { emitAutoStorage('contactout-enterprise-ratings-v1'); });
    await page.waitForFunction(() => autoMarks >= 2);
    assert.equal(await page.evaluate(() => document.querySelector('#auto-one [data-contactout-review-badge]') === firstAutoBadge), true, 'unchanged conclusions keep the existing badge node instead of clearing and repainting it');
    ...
    await page.waitForFunction(previous => autoMarks > previous && !document.querySelector('#auto-one [data-contactout-review-badge]'), marksBeforeTextChange);
    ...
    assert.equal(await page.locator('#auto-online [data-contactout-review-badge]').count(), 0, 'manual autopilot is disabled when online scoring is enabled');
