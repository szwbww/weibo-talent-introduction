...
- 计划/审计里凡写出具体计数（「N 个调用点」）或全称判断（「仅」「唯一」「无其他调用方」），必须贴 grep 命令与输出；通读文件形成的印象不算证据。三个常见陷阱：同名字符串 key 混入变量清理清单、只 grep `src/main` 漏掉测试里的 Mockito stub、框架能力假设在本仓库零先例（如 Spring Data JDBC `@Query` 返回 DTO 投影）。(K-plan-quantified-claims-need-grep-receipts)
- 「地区」是 `CountryContinentMapping` 的 9 个固定大区**英文常量**，参与 ES term 构造（`toRegion`/`countriesForRegion`/`esTermVariants`）；中文化只能改前端显示标签，`option.value`、query param、DB 值、ES term 值必须保持英文原串，否则筛选静默命中 0 条。(K-region-constant-not-display-label)
- `UNCLASSIFIED` 学科 = `disciplineCategory` 字段**不存在**，权威实现只有 `ExpertSearchService.disciplineFilter()`；另有 3 条 `term` 旁路（`buildEsFiltersForLevel` else 分支、`RecipientScope.matchesExpert`、`buildMaterialReminderEsFilters`）和 2 处不含 `UNCLASSIFIED` 的 `ALLOWED_DISCIPLINES` 白名单，补前端 option 前必须先打通这 5 处。(K-discipline-unclassified-filter-bypasses)
