...
Conversation status (`common/domain/ConversationStatus`) flows `NEW → INTRO_SENT → WAITING_REPLY → {QA_AUTO_REPLIED | MEETING_INVITATION_SENT → WAITING_MEETING_CONFIRMATION → MANUAL_HANDOFF} | CLOSED`. `MANUAL_HANDOFF` is the single "needs a human" state — both the inbound auto-pipeline (unclear intent, attachments, paused auto-reply, QA miss…) and operator-triggered "转人工" land on it, and both paths create a `manual_handoff` ticket so the "完成人工" workflow always has something to close. **Always transition state through `ConversationStateService.transition(...)`** — it persists the contact and appends an `ExpertContactStatusHistory` row in one place. Don't mutate `currentStatus` directly.
