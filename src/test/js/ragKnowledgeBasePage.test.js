...
describe("RAG 知识库页 (plan 04)", () => {
    ...
    it("G-5：三处 ?v= 缓存键同值且等于 20260902-rag-knowledge-base", () => {
        ["styles.css", "trust-reply-workbench.js", "app.js"].forEach((asset) => {
            assert.ok(html.includes(`${asset}?v=20260902-rag-knowledge-base`), `${asset} key`);
        });
        const keys = [...html.matchAll(/\?v=([0-9a-z-]+)/g)].map((match) => match[1]);
        assert.ok(keys.length >= 3, `expected 3+ cache keys, got ${keys.length}`);
        assert.ok(keys.every((key) => key === "20260902-rag-knowledge-base"), `all keys must share one value: ${keys}`);
    });
