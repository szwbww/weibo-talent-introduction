...
//    world-clock.css/.js），全部同值 20260917-calendar-layout-align，无重复注册；
...
describe("T3: 11 个带版本资源统一键与注册（I-1/S-1）", () => {
    it("恰好 11 个带 ?v= 资源且全部等于 20260917-calendar-layout-align，无旧键残留", () => {
    ...
    it("link 全在 head、script 全在 body；task-modal-runtime.js 未版本化且位于组件脚本之前", () => {
        ...
        const linkRe = /<link rel="stylesheet" href="[^"]+\.css\?v=20260917-calendar-layout-align">/g;
        const scriptRe = /<script src="[^"]+\.js\?v=20260917-calendar-layout-align"><\/script>/g;
        let match;
...
describe("T4: 02 注册新资源与顶栏入口回归（I-7/I-8/S-5/S-6）", () => {
    ...
    it("导航 data-view 集合与顺序：会议日历插入邮箱之后，其余不变（I-1/S-5）", () => {
        ...
        assert.deepStrictEqual(views, ["monitoring", "accounts", "mail-templates", "suppressions",
            "contacts", "mailbox", "meeting-calendar", "inbound-summary", "ai-training", "tasks"],
            "会议日历必须紧随 mailbox 之后插入，其余 data-view 的集合与顺序不得变化");
    });
});
