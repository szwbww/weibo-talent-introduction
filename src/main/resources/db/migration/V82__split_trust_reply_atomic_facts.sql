...
-- write when either audited legacy record (id 17 / id 34) drifted from the
-- 2026-08-04 baseline -- id, reply_subject, exact keywords, updated_at and
-- SHA-256 of answer_body. A mismatch means operator edits landed after the
-- plan; best-effort skip or partial application is prohibited -- deployment
-- stops and the drift is merged manually. On a matching baseline the two
-- composite rules are disabled and the four atomic rules below are inserted
-- idempotently (NOT EXISTS on reply_subject) with non-overlapping keywords and
-- one semantic family per rule (I-1/I-3/I-4). reply_body = answer_body.
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_documentation_count FROM qa_rule
     WHERE id = 17
       AND reply_subject = 'Document confidentiality and no fees'
       AND keywords = 'confidential,keep my documents,never charge,any fee,money transfer'
       AND enabled = 1
       AND updated_at = '2026-06-26 22:14:06'
    ...
    SELECT COUNT(*) INTO v_contract_count FROM qa_rule
     WHERE id = 34
       AND reply_subject = 'Contract and IP arrangements'
       ...
       AND enabled = 1
       AND updated_at = '2026-07-16 18:03:00'
...
SELECT
    (SELECT category_id FROM qa_rule WHERE id = 17),
...
SELECT
    (SELECT category_id FROM qa_rule WHERE id = 34),
