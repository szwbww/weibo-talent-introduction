...
class OpenAlexDataSource(
    ...
    override fun searchPapers(criteria: PaperSearchCriteria): PaperSearchResult {
        ...
        if (criteria.cursor != null) {
            url += "&cursor=${criteria.cursor}"
        } else {
            url += "&cursor=*"
        }

        if (properties.politeEmail.isNotBlank()) {
            url += "&mailto=${properties.politeEmail}"
        }
    ...
    private fun buildFilter(criteria: PaperSearchCriteria): String {
        ...
        if (criteria.excludeCountries.isNotEmpty()) {
            for (country in criteria.excludeCountries) {
                parts += "authorships.institutions.country_code:!$country"
            }
        }

        if (criteria.keywords.isNotEmpty()) {
            val kw = criteria.keywords.joinToString("|")
            parts += "title_and_abstract.search:$kw"
        }
        ...
        val papers = response.path("results").mapNotNull { node ->
            try {
                ...
                val authors = node.path("authorships").map { authorship ->
                    ...
                    val orcid = author.path("orcid").asText(null)
                    ...
                    val affiliation = institution?.path("display_name")?.asText(null)

                    PaperAuthor(
                        givenNames = nameParts.getOrNull(0),
                        familyNames = nameParts.getOrNull(1),
                        orcidId = orcid,
                ...
                PaperMetadata(
                    ...
                    pubYear = node.path("publication_year").asInt(0),
                    journal = node.path("primary_location").path("source").path("display_name").asText(null),
                    authors = authors,
                    source = sourceName
                )
            } catch (e: Exception) {
                log.debug("Failed to parse OpenAlex work: {}", e.message)
                null
            }
        }
