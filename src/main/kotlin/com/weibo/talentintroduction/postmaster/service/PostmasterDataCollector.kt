...
class GoogleDomainStatsFetcher(
    ...
    private val oauthService: PostmasterOAuthService
) : DomainStatsFetcher {
    ...
    override fun fetch(domain: String, date: LocalDate): CollectedDomainStats? {
        return try {
            ...
            // 带上异常本身：鉴权/权限类失败（scope 不对、域名未在 Postmaster 验证、用户无权限）
    ...
    private fun buildClient(): PostmasterTools {
        val credentials = oauthService.loadCredentials()
            ?: throw IllegalStateException("Postmaster OAuth token is not configured")
        val transport = GoogleNetHttpTransport.newTrustedTransport()
        return PostmasterTools.Builder(
            transport,
            GsonFactory.getDefaultInstance(),
            com.google.auth.http.HttpCredentialsAdapter(credentials)
        )
