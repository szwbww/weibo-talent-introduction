...
object AiReplyValidationCodes {
    ...
    // 13-letter-orchestrator 编排校验码（T-3）：六道服务端纯函数校验中的五道；
    // 动作对账（G5）复用上面的既有 ACTION_* 码。
    const val ORCH_FACT_ID_UNKNOWN = "AI_REPLY_ORCH_FACT_ID_UNKNOWN"
    const val ORCH_REQUIRED_FACT_COUNT_INVALID = "AI_REPLY_ORCH_REQUIRED_FACT_COUNT_INVALID"
    const val ORCH_VERBATIM_BODY_MISSING = "AI_REPLY_ORCH_VERBATIM_BODY_MISSING"
    const val ORCH_ACTION_IN_PARAGRAPH = "AI_REPLY_ORCH_ACTION_IN_PARAGRAPH"
    const val ORCH_PLAN_MISMATCH = "AI_REPLY_ORCH_PLAN_MISMATCH"
}
