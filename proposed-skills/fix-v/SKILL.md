...
- **Approved plan:** The plan explicitly selected by the user or current task as the verification baseline, or recorded as approved in the repository. This approves only the verification baseline, not later amendments. If no approved baseline can be established, select an early blocked outcome.
- **Demonstrated production invariant:** A runtime rule supported by an explicit user statement, approved documentation, a reproducible runtime scenario, an acceptance test tied to approved behavior, logs, or incident evidence. Verifier inference alone is insufficient.
...
- No approved verification baseline can be established.
...
- A required build/test remains `FAIL` or `BLOCKED`, no actionable P1 repair applies, and permitted mechanical repair cannot resolve it.
